...
Design patterns are general, reusable solutions to commonly occurring problems in software design. They aren’t a finished design that can directly be turned into code, rather they are a description or template you can use to solve a problem in many different contexts. 

This means they can help speed up the development process as they provide tested, proven best practices that you can use, thereby improving code readability and avoiding problems down the line. They also enable the use of common names for software interactions between developers and can be improved over time, making them more robust than ad-hoc designs.

Design patterns don’t go directly into your code, they first go into your BRAIN. Once you’ve loaded your brain with a good working knowledge of patterns, you can then start to apply them to your new designs and rework your old code when you need to update it. 
...
```
That's not the right apporach. We don't want to add new methods to the super class. What we want is to separate what makes subclasses different from the the super class.
...
So, if we want to be able to change the flyingType dynamically we need to do it using the following approach:
...
```
This doesn't quite work either. We need to make sure to abstract what is different to the subclasses. So we set a constructor to initialise all objects:
...
So, to separate Dog and Bird instances in the Animal main section and check whether each animal can fly or not, we refer to the Flys interface and then set it to not fly. 
...
As we can see, Strategy lets the algorithm vary independently from the clients that use it. We can use it to encapsulate behavior into a different set of classes that can be easily expanded and changed, as needed during runtime. 

It provides a flexible alternative to subclassing; meaning you can change the behavior of a class by composing a different object within.
